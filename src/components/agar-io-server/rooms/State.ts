...
import { WORLD_SIZE, DEFAULT_PLAYER_RADIUS } from '../utils/AgarIOCostanzo';
...
export class State extends Schema {
  ...
  createFood() {
    const radius = Math.max(4, Math.random() * (DEFAULT_PLAYER_RADIUS / 3 - 1));
      ...
      if (entity.radius >= DEFAULT_PLAYER_RADIUS) {
        for (const collideSessionId in this.entities) {
          ...
          if (
            ...
            entity.distance(collideTestEntity) <=
